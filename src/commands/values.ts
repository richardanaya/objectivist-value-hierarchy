...
export interface EditValueOptions {
  title?: string
  tags?: string
  desc?: string
}
...
export async function readValues(filePath: string): Promise<Value[]> {
  ...
  let currentValue: Partial<Value> | null = null
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (valueMatch) {
      ...
      if (currentValue && currentValue.id) {
        ...
        values.push(currentValue as Value)
      }
...
export async function addValue(filePath: string, options: AddValueOptions): Promise<{ success: boolean; message: string }> {
...
export async function editValue(filePath: string, id: string, options: EditValueOptions): Promise<{ success: boolean; message: string }> {
  if (!await fs.pathExists(filePath)) {
...
export async function removeValue(filePath: string, id: string): Promise<{ success: boolean; message: string }> {
  if (!await fs.pathExists(filePath)) {
    throw new Error(`File "${filePath}" does not exist.`)
  }
...
export async function getSuggestions(filePath: string, num: number): Promise<unknown> {
  if (!await fs.pathExists(filePath)) {
    throw new Error(`File "${filePath}" does not exist.`)
  }
  ...
  const lopsidedValues = values
    .filter(v => {
      ...
      if (total < 5) return false // Need at least 5 comparisons
  ...
  if (values.length >= 2) {
    ...
    const selectedPairs: [Value, Value][] = []
...
export async function setHigherPriorityThan(filePath: string, valueTitle: string, valueToBeAboveTitle: string): Promise<{ success: boolean; message: string }> {
  if (!await fs.pathExists(filePath)) {
    throw new Error(`File "${filePath}" does not exist.`)
  }
...
export async function listValues(filePath: string, options: ListOptions): Promise<unknown> {
  if (!await fs.pathExists(filePath)) {
    throw new Error(`File "${filePath}" does not exist.`)
  }
